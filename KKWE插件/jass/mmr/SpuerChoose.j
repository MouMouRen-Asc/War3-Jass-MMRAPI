...
library MSuperChoose requires MmrApi
    ...
    function MSC_ReCalcuteFrameByShow takes integer uinum ,real wlength ,real hlength  returns nothing
        ...
        call DzFrameShow(ReFresh[0],PCanRe[GetPlayerId(GetLocalPlayer())])
        call DzFrameShow(ReFresh[1],PCanRe[GetPlayerId(GetLocalPlayer())])

    endfunction

    function MSC_DisLikeShow takes boolean b returns nothing
        set CanClose = b
    endfunction

    function MSC_PreShow takes player p , boolean b returns nothing
        set PCanRe[GetPlayerId(p)] = b
    endfunction
    ...
    function MSC_Create_ChooseUi takes integer ChooseNumber returns nothing
        ...
        if CanClose then
            ...
            call DzFrameShow(DisLike[0],false)
            call DzFrameShow(DisLike[1],false)
        endif
    ...
    function MSC_CloseUiAsPlayer takes player p returns nothing
        if p == null then
            return
        endif
        if p == GetLocalPlayer() then
