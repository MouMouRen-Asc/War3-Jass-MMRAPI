...
//扇形选取并造成伤害
    function XIAOWU_SXXQ_A takes unit a,  unit targetunit , real b,real c,real d,boolean e,boolean f,attacktype g, damagetype h returns nothing
        local group dwz
        local unit dw
        set dwz = CreateGroup()
        call GroupEnumUnitsInRange(dwz,GetUnitX(targetunit),GetUnitY(targetunit),b,null)
        loop
...
//任意矩形选取
    function XIAOWU_JXXQ_A takes unit aa, unit targetunit, location b,real c,real d,boolean e,boolean f,attacktype g, damagetype h returns nothing
        local location a = GetUnitLoc(targetunit)
